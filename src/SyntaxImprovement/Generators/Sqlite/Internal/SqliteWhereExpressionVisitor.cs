...
{
	internal class SqliteWhereExpressionVisitor<TableType> : ExpressionVisitor where TableType : SqliteDatabaseTable, new()
	{
		private readonly SqliteParameterFactory sqliteParameterFactory;
		private readonly StringBuilder stringBuilder;
		private readonly Stack<ExpressionType> operatorStack;

		internal SqliteWhereExpressionVisitor(SqliteParameterFactory sqliteParameterFactory)
		{
			this.sqliteParameterFactory = sqliteParameterFactory;
		...
		public override Expression Visit(Expression node)
		{
			...
			{
				var argumentVisitor = new SqliteWhereExpressionVisitor<TableType>(new SqliteParameterFactory());
				argumentVisitor.Visit(methodCallExpression.Arguments);

				var callVisitor = new SqliteWhereExpressionVisitor<TableType>(new SqliteParameterFactory());
				...
				if (methodCallExpression.Method.DeclaringType == typeof(string))
				{
					...
					stringBuilder.Append(sqliteParameterFactory.Create("%" + argumentVisitor.sqliteParameterFactory.parameters.Single().Value + "%"));
					return node;
				}
				else if (methodCallExpression.Method.DeclaringType.IsGenericType && typeof(IEnumerable).IsAssignableFrom(methodCallExpression.Method.DeclaringType))
				{
					...
					for (var i = 0; i < callVisitor.sqliteParameterFactory.parameters.Count; i++)
					{
						var parameter = callVisitor.sqliteParameterFactory.parameters[i];
						var value = parameter.Value;
						var valueExpression = sqliteParameterFactory.Create(value);
						stringBuilder.Append(valueExpression);
						if (i != callVisitor.sqliteParameterFactory.parameters.Count - 1)
						{
							stringBuilder.Append(',');
							stringBuilder.Append(' ');
						}
			...
			if (node is MemberExpression m)
			{
				...
				var parameterizedValue = sqliteParameterFactory.Create(value);
