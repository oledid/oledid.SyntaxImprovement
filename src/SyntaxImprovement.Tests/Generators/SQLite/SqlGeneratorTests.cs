	...
	public class SqlGeneratorTests
	{
		public class SelectTests
		{
			...
			{
				{
					...
					Assert.Equal("SELECT \"Id\", \"Name\" FROM \"Person\" WHERE \"Name\" IS :p0;", query.QueryText);
...

				{
					...
					Assert.Equal("SELECT \"Id\", \"Name\" FROM \"Person\" WHERE \"Name\" IS NOT :p0;", query.QueryText);
					Assert.Equal("Peter", ((IDictionary<string, object>)((dynamic)query).Parameters)[":p0"]);
				}
			}
			...
			public void It_generates_correct_where_with_multiple_arguments()
			{
				...
				Assert.Equal("SELECT \"Id\", \"Name\" FROM \"Person\" WHERE (\"Name\" IS :p0) AND (\"Id\" IS :p1);", query.QueryText);
				Assert.Equal("Peter", ((IDictionary<string, object>)((dynamic)query).Parameters)[":p0"]);
			...
			{
				{
					...
					Assert.Equal("SELECT \"IdStr\", \"IsActive\" FROM \"BooleanTestModel\" WHERE (\"IdStr\" IS :p0) AND (\"IsActive\" IS :p1);", query.QueryText);
...

				{
					...
					Assert.Equal("SELECT \"Id\", \"IsDeleted\" FROM \"LongTest\" WHERE (\"Id\" IS :p0) AND (\"IsDeleted\" IS :p1);", query.QueryText);
...

				{
					...
					Assert.Equal("SELECT \"Id\", \"Fk\", \"IsDeleted\" FROM \"GuidTestEntity\" WHERE (\"Fk\" IS :p0) AND (\"IsDeleted\" IS :p1);", query.QueryText);
					Assert.Equal(id, ((IDictionary<string, object>)((dynamic)query).Parameters)[":p0"]);
					Assert.Equal(false, ((IDictionary<string, object>)((dynamic)query).Parameters)[":p1"]);
				}
			...
			public void It_understands_gt_and_lt()
			{
				...
				Assert.Equal("SELECT \"Id\", \"IsDeleted\" FROM \"LongTest\" WHERE ((\"Id\" > :p0) AND (\"IsDeleted\" IS :p1)) AND (\"Id\" <= :p2);", query.QueryText);
			...
			public void It_understands_boolean_true()
			{
				var query = new Select<BooleanTestModel>()
					.Where(e => e.IsActive)
					.ToQuery();
				Assert.Equal("SELECT \"IdStr\", \"IsActive\" FROM \"BooleanTestModel\" WHERE \"IsActive\" IS :p0;", query.QueryText);
		...
		public class UpdateTests
		{
			...
			public void It_generates_correct_update()
			{
				...
				Assert.Equal("UPDATE \"Person\" SET \"Name\" = :p1 WHERE \"Id\" IS :p0", query.QueryText);
			...
			public void It_generates_correct_update_IN()
			{
				...
				Assert.Equal("UPDATE \"Person\" SET \"Name\" = :p4 WHERE (\"Name\" IN (:p0, :p1, :p2)) AND (\"Id\" IS :p3)", query.QueryText);
			...
			public void It_generates_correct_update_with_multi_params()
			{
				...
				Assert.Equal("UPDATE \"Person\" SET \"Name\" = :p1, \"Id\" = :p2 WHERE \"Id\" IS :p0", query.QueryText);
				Assert.Equal("Peter", ((IDictionary<string, object>)((dynamic)query).Parameters)[":p1"]);
			...
			public void It_can_update_entire_model()
			{
				...
				Assert.Equal("UPDATE \"Table\" SET \"Id\" = :p1, \"FirstName\" = :p2, \"IsAdmin\" = :p4, \"LastName\" = :p5 WHERE \"Id\" IS :p0", query.QueryText);
		...
		public class InsertTests
		{
			...
			public void It_generates_correct_insert_for_single_object_with_identity_column()
			{
				...
				Assert.Equal("INSERT INTO \"Person\" (\"Name\") SELECT :p0; SELECT last_insert_rowid();", query.QueryText);
				Assert.Equal("Peter", ((IDictionary<string, object>)((dynamic)query).Parameters)[":p0"]);
			}
		...
		public class DeleteTests
		{
			...
			public void It_generates_correct_query()
			{
				...
				Assert.Equal("DELETE FROM \"Person\" WHERE (\"Name\" IS :p0) OR (\"Id\" IS :p1);", query.QueryText);
				Assert.Equal("Peter", ((IDictionary<string, object>)((dynamic)query).Parameters)[":p0"]);
				Assert.Equal(1, ((IDictionary<string, object>)((dynamic)query).Parameters)[":p1"]);
			}
		}
		...
		public class NullTests
		{
			...
			public void It_handles_NULL_in_WHERE()
			{
				...
				var expected = $"SELECT \"Id\", \"FkId\", \"NullableDateTime\", \"NullableBool\", \"NullableStr\" FROM \"NullTest\" WHERE (\"FkId\" IS :p0) AND (((\"NullableDateTime\" IS NULL) OR ((\"NullableBool\" IS :p1) AND (\"NullableStr\" IS NULL))) OR (\"NullableStr\" IS :p2));";
			...
			public void It_handles_NOT_NULL_in_WHERE()
			{
				...
				var expected = $"SELECT \"Id\", \"FkId\", \"NullableDateTime\", \"NullableBool\", \"NullableStr\" FROM \"NullTest\" WHERE (\"FkId\" IS :p0) AND (((\"NullableDateTime\" IS NOT NULL) OR ((\"NullableBool\" IS :p1) AND (\"NullableStr\" IS NULL))) OR (\"NullableStr\" IS :p2));";
				Assert.Equal(expected, query.QueryText);
				Assert.Equal(1, ((IDictionary<string, object>)((dynamic)query).Parameters)[":p0"]);
				Assert.Equal(false, ((IDictionary<string, object>)((dynamic)query).Parameters)[":p1"]);
				Assert.Equal("123", ((IDictionary<string, object>)((dynamic)query).Parameters)[":p2"]);
